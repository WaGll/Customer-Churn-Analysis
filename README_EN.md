...
### 6. 预测建模 🆕
- **多模型对比** - Logistic Regression、Random Forest、XGBoost
- **自动超参调优** - GridSearchCV + Stratified K-Fold
- **自动模型选择** - 基于 ROC-AUC 选择最佳模型
- **模型可解释性** - SHAP 特征重要性分析
- **模型持久化** - 自动保存最佳模型 (output/models/churn_model.pkl)
- **API 服务** - FastAPI 实时预测接口 (POST /predict)
...
print(f"ROC-AUC: {summary['best_metrics']['roc_auc']:.4f}")
```
